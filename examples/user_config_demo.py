...
async def demo_user_config():
    ...
    from tldwatch.core.user_config import UserConfig, get_user_config, reload_user_config
    ...
    try:
        ...
        provider = UnifiedProvider()
        print(f"   Default: {provider.config.name}, {provider.model}, temp={provider.temperature}, chunking={provider.chunking_strategy.value}")
        
        # Test specific providers
        openai_provider = UnifiedProvider(provider="openai")
        print(f"   OpenAI: {openai_provider.config.name}, {openai_provider.model}, temp={openai_provider.temperature}")
        
        google_provider = UnifiedProvider(provider="google")
        print(f"   Google: {google_provider.config.name}, {google_provider.model}, temp={google_provider.temperature}")
        ...
        print(f"   Override: {override_provider.config.name}, {override_provider.model}, temp={override_provider.temperature}, chunking={override_provider.chunking_strategy.value}")
        ...
        print(f"     Provider: {temp_provider.config.name}, Model: {temp_provider.model}")
        ...
        print(f"     Provider: {temp_provider.config.name}, Temperature: {temp_provider.temperature}")
...
"""
        ...
        reload_user_config()
        provider = UnifiedProvider()
        print(f"   YAML config: {provider.config.name}, {provider.model}, temp={provider.temperature}, chunking={provider.chunking_strategy.value}")
