...
class UnifiedProvider:
    ...
    def _load_providers(cls) -> Dict[str, ProviderConfig]:
        """Load provider configurations from config.yaml file"""
        ...
        try:
            ...
            logger.error(f"Failed to load provider configuration from config.yaml: {e}")
            # Fallback to empty dict - will raise error in initialization
            return {}

    @property
    def PROVIDERS(self) -> Dict[str, ProviderConfig]:
        """Get provider configurations (loaded from config.yaml)"""
    ...
    def __init__(
        ...
        if provider not in self.PROVIDERS:
            raise ValueError(
                f"Unsupported provider: {provider}. Choose from: {list(self.PROVIDERS.keys())}"
            )
        ...
        if chunking_strategy is None:
            chunking_strategy = (
                user_config.get_default_chunking_strategy() or "standard"
            )
    ...
    def list_providers(cls) -> List[str]:
        ...
        return list(cls._load_providers().keys())
